...
public final class Field {
    ...
    private final CriticalTemperature criticalTemperature;

    public Field(FieldId id, FieldName name, Crop crop,
                 CriticalTemperature criticalTemperature) {
        this.id = Objects.requireNonNull(id, "id must not be null");
        this.name = Objects.requireNonNull(name, "name must not be null");
        this.crop = Objects.requireNonNull(crop, "crop must not be null");
        this.criticalTemperature = Objects.requireNonNull(
                criticalTemperature, "criticalTemperature must not be null");
    }
    ...
    public CriticalTemperature criticalTemperature() {
        return criticalTemperature;
    }

    public FrostAssessment assessFrost(MeasuredTemperature measuredTemperature) {
        Objects.requireNonNull(measuredTemperature, "measuredTemperature must not be null");

        double measured = measuredTemperature.value();
        double critical = criticalTemperature.value();
        FrostRiskLevel riskLevel = measured <= critical
                ? FrostRiskLevel.CRITICAL
                : measured <= critical + WARNING_MARGIN_CELSIUS
                        ? FrostRiskLevel.WARNING
                        : FrostRiskLevel.SAFE;

        return new FrostAssessment(id, measuredTemperature, criticalTemperature, riskLevel);
    }
